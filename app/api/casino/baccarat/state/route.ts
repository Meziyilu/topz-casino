...
// 建下一局（⚠️ 不要給 tx 註明型別，讓 Prisma 自推斷，避免型別不合）
async function createNextRoundTx(tx: any, roomId: string, dayStartUtc: Date) {
  ...
  return tx.round.create({
    ...
    select: {
      id: true, createdAt: true, roundSeq: true, phase: true, startedAt: true,
      outcome: true, playerTotal: true, bankerTotal: true,
    },
  });
}

async function settleRoundTx(tx: any, roundId: string) {
    ...
    return noStoreJson({
      ...
      result:
        phase === "SETTLED"
          ...
          : null,
      myBets,
      recent: recentRows.map((x) => ({
        roundSeq: x.roundSeq, outcome: x.outcome, p: x.playerTotal ?? 0, b: x.bankerTotal ?? 0,
      })),
    });
  } catch (e: any) {
