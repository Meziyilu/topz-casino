...
export default function LobbyPage() {
  return (
    <main style={{ minHeight:'100svh', display:'grid', placeItems:'center',
      ...
      <div style={{
        width:'min(960px,92vw)', padding:24, borderRadius:16,
        background:'rgba(16,20,27,.45)', border:'1px solid rgba(255,255,255,.12)',
        boxShadow:'0 10px 30px rgba(0,0,0,.5), inset 0 1px 0 rgba(255,255,255,.08), 0 0 80px rgba(0,180,255,.12)',
        backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)', color:'#dce3ea'
      }}>
        <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginBottom:12}}>
          <div style={{ fontSize:20, letterSpacing:2, fontWeight:800 }}>TOPZCASINO</div>
        ...
        <div style={{ fontSize:28, fontWeight:700, marginTop:8 }}>大廳已就緒 ✅</div>
        <p style={{ opacity:.8, marginTop:8 }}>
          這是「安全版」大廳骨架。之後再慢慢把遊戲卡片 / 聊天室加回來。
        </p>
      </div>
    </main>
  );
