...
function baselineState(room:RoomKey, cfg:Cfg): RoomState {
  const now = new Date();
  const locksAt = new Date(now.getTime() + (cfg.drawIntervalSec - cfg.lockBeforeRollSec)*1000);
  return { room, roundId: null, day: todayISO(), daySeq: 0, phase: "BETTING", startsAt: now.toISOString(), locksAt: locksAt.toISOString(), settledAt: null, exposure: {} };
}

function roll(): [number,number,number]{ return [1+Math.floor(Math.random()*6),1+Math.floor(Math.random()*6),1+Math.floor(Math.random()*6)]; }
...
async function createRound(room:RoomKey, r:RoomInternal){
  ...
  const round = await prisma.sicboRound.create({
    data: { room, day: new Date(day+"T00:00:00.000Z"), daySeq, phase: "BETTING",
            startsAt, locksAt, die1:0, die2:0, die3:0, sum:0, isTriple:false, serverSeed, seedHash }
  });

  r.state = { room, roundId: round.id, day, daySeq, phase:"BETTING", startsAt: startsAt.toISOString(), locksAt: locksAt.toISOString(), settledAt: null, exposure: {} };
  sicboHub.emit(topic(room, "state"), r.state);
}
...
async function settleRound(room:RoomKey, r:RoomInternal){
  ...
  const round = await prisma.sicboRound.update({
    where: { id: r.state.roundId },
    data: { phase:"SETTLED", die1:d1, die2:d2, die3:d3, sum, isTriple, settledAt: new Date() }
  });

  const bets = await prisma.sicboBet.findMany({ where: { roundId: round.id, status: "PLACED" }});
  const updates = [];
  for (const b of bets) {
    const odds = Number(b.odds), amount = b.amount;
    let win = 0;

    if (b.kind==="BIG_SMALL" && b.bigSmall) {
      const big = sum>=11 && sum<=17, small = sum>=4 && sum<=10;
      win = (isTriple) ? 0 : ((b.bigSmall==="BIG" && big) || (b.bigSmall==="SMALL" && small) ? Math.floor(amount*odds) : 0);
    } else if (b.kind==="TOTAL" && b.totalSum!=null) {
      win = (sum===b.totalSum) ? Math.floor(amount*odds) : 0;
    } else if (b.kind==="SINGLE_FACE" && b.face!=null) {
      const c = [d1,d2,d3].filter(v=>v===b.face).length;
      win = c>0 ? amount * ({1:1,2:2,3:3} as any)[c] : 0;
    } else if (b.kind==="DOUBLE_FACE" && b.face!=null) {
      const c = [d1,d2,d3].filter(v=>v===b.face).length;
      win = c>=2 ? Math.floor(amount*odds) : 0;
    } else if (b.kind==="ANY_TRIPLE") {
      win = isTriple ? Math.floor(amount*odds) : 0;
    } else if (b.kind==="SPECIFIC_TRIPLE" && b.face!=null) {
      win = (isTriple && d1===b.face) ? Math.floor(amount*odds) : 0;
      ...
      win = has.has(key) ? Math.floor(amount*odds) : 0;
    }

    if (win>0) {
      updates.push(prisma.ledger.create({ data: { type:"PAYOUT", game:"SICBO", gameRef: b.id, amount: win, userId: b.userId }}));
      updates.push(prisma.user.update({ where:{ id:b.userId }, data:{ balance: { increment: win }}}));
    }
    updates.push(prisma.sicboBet.update({ where:{ id:b.id }, data: { payout: win, status:"SETTLED", settledAt:new Date() } }));
  }
  if (updates.length) await prisma.$transaction(updates);

  r.state.phase = "SETTLED"; r.state.dice = [d1,d2,d3]; r.state.sum = sum; r.state.isTriple = isTriple; r.state.settledAt = new Date().toISOString();
  sicboHub.emit(topic(room, "result"), { dice:[d1,d2,d3], sum, isTriple, roundId: round.id });
  sicboHub.emit(topic(room, "state"), r.state);
}
...
async function tick(room:RoomKey, r:RoomInternal){
  ...
  if (r.state.phase!=="SETTLED" && now.getTime() >= new Date(r.state.startsAt).getTime() + r.cfg.drawIntervalSec*1000 - 50) {
...
async function loop(room:RoomKey, r:RoomInternal){
  clearTimeout(r.timer!);
  r.timer = setTimeout(async ()=> {
    try { await tick(room, r); } finally { loop(room, r); }
  }, 1000);
}
...
export async function ensureRooms(){
  for (const room of ["R30","R60","R90"] as RoomKey[]) {
    if (!rooms.has(room)) {
      ...
      const r: RoomInternal = { state: baselineState(room, cfg), cfg, timer: null };
      rooms.set(room, r);
      await createRound(room, r);
      loop(room, r);
    }
...
export function subscribeRoom(room:RoomKey, channel:string, handler:(data:any)=>void){ return sicboHub.subscribe(`sicbo:${room}:${channel}`, handler); }
