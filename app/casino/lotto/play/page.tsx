// app/casino/lotto/play/page.tsx
"use client";

import { useEffect, useMemo, useState } from "react";
import { useMe } from "@/components/useMe";
import { useCss } from "@/components/useCss";

type DrawLite = {
  id: string;
  code: number;
  drawAt: string;
  status: "OPEN" | "LOCKED" | "DRAWN" | "SETTLED";
  numbers: number[];
  special: number | null;
  pool: number;
  jackpot: number;
};
type StateResp = {
  current: DrawLite | null;
  last: { id: string; code: number; numbers: number[]; special: number | null } | null;
  config: {
    drawIntervalSec: number;
    lockBeforeDrawSec: number;
    picksCount: number;
    pickMax: number;
    betTiers: number[];
  };
};

export default function LottoPlay() {
  useCss("/styles/lotto.css");

  const { me, loading: meLoading, reload: reloadMe } = useMe(0);
  const [s, setS] = useState<StateResp | null>(null);

  const [amount, setAmount] = useState<number>(100);
  const [picked, setPicked] = useState<number[]>([]);
  const [placing, setPlacing] = useState(false);
  const [toast, setToast] = useState<string>("");

  // 尾數玩法
  const [tailParity, setTailParity] = useState<"ODD" | "EVEN" | null>(null);
  const [tailSize, setTailSize] = useState<"BIG" | "SMALL" | null>(null);

  const [myBets, setMyBets] = useState<{ picks: number[]; amount: number; createdAt: string }[]>([]);
  const [history10, setHistory10] = useState<any[]>([]);

  const pickMax = s?.config.pickMax ?? 49;
  const picksCount = s?.config.picksCount ?? 6;

  const options = useMemo(() => Array.from({ length: pickMax }, (_, i) => i + 1), [pickMax]);

  async function pullState() {
    const r = await fetch("/api/casino/lotto/state", { cache: "no-store" });
    setS(await r.json());
  }
  async function pullHistory10() {
    const r = await fetch("/api/casino/lotto/history?take=10&includeDrawn=1", { cache: "no-store" });
    const j = await r.json();
    setHistory10(j.items || []);
  }
  async function pullMyBets(uid: string) {
    const r = await fetch(`/api/casino/lotto/my-bets?userId=${encodeURIComponent(uid)}`, { cache: "no-store" });
    const j = await r.json();
    setMyBets(
      (j.items || [])
        .slice(0, 10)
        .map((it: any) => ({ picks: it.picks, amount: it.amount, createdAt: it.createdAt }))
    );
  }

  useEffect(() => {
    pullState();
    pullHistory10();
    const t = setInterval(() => {
      pullState();
      pullHistory10();
    }, 1000);
    return () => clearInterval(t);
  }, []);

  useEffect(() => {
    if (me?.id) pullMyBets(me.id);
  }, [me?.id]);

  function toggle(n: number) {
    if (picked.includes(n)) setPicked(picked.filter((x) => x !== n));
    else if (picked.length < picksCount) setPicked([...picked, n].sort((a, b) => a - b));
  }

  function quick(n: number) {
    const nums: number[] = [];
    const pool = [...options];
    for (let i = 0; i < picksCount; i++) {
      const idx = Math.floor(Math.random() * pool.length);
      nums.push(pool[idx]);
      pool.splice(idx, 1);
    }
    nums.sort((a, b) => a - b);
    setPicked(nums);
    setAmount(n);
  }

  async function place() {
    if (!me?.id) {
      setToast("尚未登入，請先登入帳號");
      return;
    }
    if (!s?.current) {
      setToast("目前無可下注場次");
      return;
    }
    if (s.current.status !== "OPEN") {
      setToast("已鎖盤");
      return;
    }
    if (picked.length !== picksCount) {
      setToast(`需選 ${picksCount} 顆`);
      return;
    }
    setPlacing(true);
    try {
      const r = await fetch("/api/casino/lotto/bet", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: me.id, picks: picked, amount, tailParity, tailSize }),
      });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "下注失敗");
      setToast("下注成功！");
      setPicked([]);
      await reloadMe();
      await pullState();
      if (me?.id) await pullMyBets(me.id);
    } catch (e: any) {
      setToast(e.message);
    } finally {
      setPlacing(false);
    }
  }

  const left = s?.current ? Math.max(0, Math.floor((new Date(s.current.drawAt).getTime() - Date.now()) / 1000)) : 0;

  // 最近開獎：優先當期 DRAWN，否則上一期
  const showDraw =
    s?.current?.status === "DRAWN"
      ? { code: s.current.code, nums: s.current.numbers ?? [], special: s.current.special ?? null }
      : s?.last
      ? { code: s.last.code, nums: s.last.numbers ?? [], special: s.last.special ?? null }
      : null;

  return (
    <main className="lotto-play glass glow">
      <header className="lotto-head">
        <h1>樂透投注</h1>
        <div className="hint">
          本期 #{s?.current?.code ?? "-"} · 倒數 <strong>{left}</strong>s · 狀態 {s?.current?.status ?? "-"}
        </div>
      </header>

      <section className="panel">
        <div className="form-row">
          <label>會員</label>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {meLoading ? (
              <span className="muted">載入中…</span>
            ) : me ? (
              <>
                <span>
                  <strong>{me.displayName || me.name || me.id}</strong>
                </span>
                <span className="muted">餘額</span>
                <span>
                  <strong>{me.balance ?? 0}</strong>
                </span>
              </>
            ) : (
              <span className="muted">未登入</span>
            )}
          </div>
        </div>

        <div className="form-row">
          <label>注金</label>
          <input type="number" value={amount} onChange={(e) => setAmount(parseInt(e.target.value || "0"))} />
          <div className="chips">
            {(s?.config.betTiers ?? [10, 50, 100, 200]).map((n) => (
              <button key={n} className="chip" onClick={() => setAmount(n)}>
                {n}
              </button>
            ))}
            <button className="chip" onClick={() => quick(100)}>
              隨機選號100
            </button>
          </div>
        </div>

        {/* 尾數玩法 */}
        <div className="form-row">
          <label>尾數單/雙</label>
          <div className="chips">
            <button className={`chip ${tailParity === null ? "picked" : ""}`} onClick={() => setTailParity(null)}>
              不選
            </button>
            <button className={`chip ${tailParity === "ODD" ? "picked" : ""}`} onClick={() => setTailParity("ODD")}>
              單
            </button>
            <button className={`chip ${tailParity === "EVEN" ? "picked" : ""}`} onClick={() => setTailParity("EVEN")}>
              雙
            </button>
          </div>
        </div>
        <div className="form-row">
          <label>尾數大/小</label>
          <div className="chips">
            <button className={`chip ${tailSize === null ? "picked" : ""}`} onClick={() => setTailSize(null)}>
              不選
            </button>
            <button className={`chip ${tailSize === "BIG" ? "picked" : ""}`} onClick={() => setTailSize("BIG")}>
              大(5-9)
            </button>
            <button className={`chip ${tailSize === "SMALL" ? "picked" : ""}`} onClick={() => setTailSize("SMALL")}>
              小(0-4)
            </button>
          </div>
        </div>

        {/* 選號 */}
        <div className="pick-grid">
          {options.map((n) => (
            <button key={n} className={`num ${picked.includes(n) ? "picked" : ""}`} onClick={() => toggle(n)}>
              {n}
            </button>
          ))}
        </div>

        <div className="picked-bar">已選：{picked.length ? picked.join(", ") : "（尚未選號）"}</div>

        <div className="actions">
          <button
            className="btn-primary"
            disabled={placing || !s?.current || s.current.status !== "OPEN" || !me?.id}
            onClick={place}
          >
            {placing ? "下注中..." : "下注"}
          </button>
          <button className="btn-ghost" onClick={() => setPicked([])}>
            清除
          </button>
        </div>

        {toast && <div className="toast">{toast}</div>}
      </section>

      {/* 最近開獎（reveal） */}
      <section className="panel">
        <h3>最近開獎（#{showDraw?.code ?? "-"})</h3>
        <div className="balls balls-reveal">
          {showDraw?.nums.map((n) => (
            <div key={`p-${n}`} className="ball reveal">
              {n}
            </div>
          ))}
          {showDraw && showDraw.special != null && (
            <div className="ball special reveal">{showDraw.special}</div>
          )}
        </div>
      </section>

      {/* 我的近期下注（快速回填） */}
      <section className="panel">
        <h3>我的近期下注</h3>
        {myBets.length === 0 ? (
          <div className="muted">尚無資料</div>
        ) : (
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {myBets.map((b, i) => (
              <button
                key={i}
                className="card"
                onClick={() => {
                  setPicked([...b.picks]);
                  setAmount(b.amount);
                }}
              >
                <div className="balls" style={{ gap: 6 }}>
                  {b.picks.map((n) => (
                    <div key={n} className="ball" style={{ width: 28, height: 28, fontSize: 12 }}>
                      {n}
                    </div>
                  ))}
                </div>
                <div className="muted" style={{ marginTop: 6 }}>
                  注金 {b.amount}
                </div>
              </button>
            ))}
          </div>
        )}
      </section>

      {/* 近 10 期開獎 */}
      <section className="panel" style={{ marginTop: 16 }}>
        <h3>近 10 期開獎</h3>
        {history10.length === 0 ? (
          <div className="muted">尚無資料</div>
        ) : (
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: 8 }}>
            {history10.map((h) => (
              <li key={h.code} className="glass" style={{ padding: 10 }}>
                <div
                  style={{
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "space-between",
                    gap: 10,
                    flexWrap: "wrap",
                  }}
                >
                  <div style={{ minWidth: 90 }}>#{h.code}</div>
                  <div className="balls" style={{ gap: 6 }}>
                    {(h.numbers || []).map((n: number) => (
                      <div key={n} className="ball" style={{ width: 28, height: 28, fontSize: 12 }}>
                        {n}
                      </div>
                    ))}
                    {h.special != null && (
                      <div className="ball special" style={{ width: 28, height: 28, fontSize: 12 }}>
                        {h.special}
                      </div>
                    )}
                  </div>
                  <div className="muted" style={{ whiteSpace: "nowrap" }}>
                    {new Date(h.drawAt).toLocaleString("zh-TW", { timeZone: "Asia/Taipei" })}
                    {h.status === "DRAWN" && (
                      <span style={{ marginLeft: 8 }} className="badge yellow">
                        未結算
                      </span>
                    )}
                  </div>
                </div>
              </li>
            ))}
          </ul>
        )}
      </section>
    </main>
  );
}
