...
export class SicboService {
  static async getState(room:SicboRoomKey, userId?:string){
    ...
    return {
      ...
      current:{ roundId:s.roundId, day:s.day, daySeq:s.daySeq, phase:s.phase,
                startsAt:s.startsAt, locksAt:s.locksAt, settledAt:s.settledAt },
      config:{ drawIntervalSec:cfg.drawIntervalSec, lockBeforeRollSec:cfg.lockBeforeRollSec,
               limits:cfg.limits, payoutTable:cfg.payout },
      exposure:s.exposure, history, my
    };
  }
}
