...
async function getCatalog() {
  const res = await fetch(absUrl("/api/shop/catalog"), {
    cache: "no-store",
  });
  if (!res.ok) throw new Error("failed");
  return (await res.json()) as { items: CatalogItem[] };
}
            ...
            <div className="shop-card-body">
              ...
              <div className="shop-card-sub">
                ...
                <span className="price">
                  {formatUnit(it.priceFrom, it.currency)} 起
                </span>
                {it.limitedQty != null && (
                  <span className="stock">剩 {it.limitedQty}</span>
                )}
              </div>
            </div>
          </Link>
