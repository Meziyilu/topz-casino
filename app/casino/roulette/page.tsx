...
import "@/public/styles/roulette-lobby.css"; // 你現有的
...
const ROOMS: { code: Room; label: string; desc: string }[] = [
  ...
  { code: "RL_R60", label: "R60", desc: "60s 投注 + 10s 揭示" },
  { code: "RL_R90", label: "R90", desc: "90s 投注 + 10s 揭示" },
];
    ...
    <div className="rlb-wrap">
      ...
      <section className="rlb-grid">
        {ROOMS.map((r) => {
          ...
          return (
            <Link key={r.code} href={`/casino/roulette/${r.code}`} className="rlb-card">
              <div className={`badge ${String(phase).toLowerCase()}`}>{phase}</div>
              <div className="wheel">
                <div className="pin" />
                <div className={`spinring ${phase === "REVEALING" ? "spin" : ""}`} />
