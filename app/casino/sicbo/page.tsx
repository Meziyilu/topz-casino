  ...
  return (
    <div className="glass p-4 rounded-lg flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <a href={`/casino/sicbo/${room.toLowerCase()}`} className="px-3 py-1 rounded bg-white/20 hover:bg-white/30">進入房間</a>
      </div>
      {data ? (
        <>
          <div className="text-sm opacity-80">局號 <b>{data.current.daySeq}</b>｜狀態 <b>{data.current.phase}</b>｜封盤倒數 <b>{countdown}s</b></div>
          <div className="text-sm opacity-80">
            上局：{last ? (
              <span>骰面 <b>{last.die1}-{last.die2}-{last.die3}</b>（總點 <b>{last.sum}</b>{last.isTriple ? "，圍骰":""}）</span>
            ) : "尚無"}
          </div>
        </>
