// app/casino/sicbo/r60/page.tsx   （r30/r90 同版，改 room="R30"/"R90"）
"use client";
...
export default function Page(){
  ...
  useSSE(`/api/casino/sicbo/stream?room=${room}`, (ev:any)=>{
    ...
    if (ev.type==="result"){
      ...
      const pairs = new Set([
        `${Math.min(dice[0],dice[1])}_${Math.max(dice[0],dice[1])}`,
        `${Math.min(dice[0],dice[2])}_${Math.max(dice[0],dice[2])}`,
        `${Math.min(dice[1],dice[2])}_${Math.max(dice[1],dice[2])}`
      ]);
      pairs.forEach((k:any)=> wins.add(`COMBO_${k}`));

      setWinKeys(wins);
      setTimeout(()=> setWinKeys(new Set()), 2400); // 約閃 4 次
      load(); // 取最新 balance/歷史
    }
  });
