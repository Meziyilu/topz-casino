  ...
  return (
    <div className="rl-room">
      ...
      <section className="rl-main">
        <div className="panel glass wheelWrap">
          <RouletteWheel size={320} phase={phase as any} result={state?.result ?? null} spinMs={10000} idleSpeed={10} />
          <RouletteBall size={360} phase={phase} result={typeof state?.result === "number" ? state!.result : undefined}
            onRevealEnd={() => {}} />
          <div className="result-chip">{state?.result != null ? `結果：${state.result}` : "等待結果…"}</div>
        </div>

        <div className="panel glass betPanel">
          <h3>快速下注</h3>
          <div className="chips">
          ...
          <div className="grid grid3">
            {[1,2,3].map(d=><button key={d} onClick={()=>place("DOZEN" as RouletteBetKind,50,{dozen:d-1})}>第{d}打</button>)}
            {[1,2,3].map(c=><button key={c} onClick={()=>place("COLUMN" as RouletteBetKind,50,{col:c-1})}>第{c}列</button>)}
          </div>
      ...
      <style jsx global>{`
        ...
        .phase.pill { padding:6px 10px; border-radius:999px; font-weight:600; background:rgba(255,255,255,.08); }
        ...
        .wheelWrap { position:relative; display:flex; flex-direction:column; align-items:center; min-height:420px; }
        .result-chip { margin-top:12px; font-weight:700; text-align:center; color:#fff; opacity:.9; }

        .betPanel h3 { margin:0 0 10px; }
        .betPanel h4 { margin:16px 0 8px; font-size:14px; opacity:.9; }
        ...
        .chip {
          ...
          background:rgba(255,255,255,.06); cursor:pointer; color:#fff; font-weight:600;
        ...
        .grid button, .nums button {
          color:#fff; font-weight:600; border:1px solid rgba(255,255,255,.1);
          border-radius:10px; padding:12px 0; background:rgba(255,255,255,.04);
          transition:transform .08s ease, background .2s;
        }
        .grid button:hover, .nums button:hover { transform:translateY(-1px); background:rgba(255,255,255,.1); }

        .nums { display:grid; gap:6px; grid-template-columns:repeat(10,1fr); max-height:260px; overflow:auto; }
        .nums button { padding:8px 0; }
        @media(max-width:960px){ .rl-main { grid-template-columns:1fr; } }
      `}</style>
    </div>
  );
}
