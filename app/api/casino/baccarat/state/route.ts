...
// ========= Taipei day window (以 UTC 計算當地日界線) =========
...
function payoutFactor(
  ...
  switch (side) {
    case "PLAYER":
      return outcome === "PLAYER" ? 2.0 : outcome === "TIE" ? 1.0 : 0;
    case "BANKER":
      return outcome === "BANKER" ? 1.95 : outcome === "TIE" ? 1.0 : 0; // 5% 抽水
    case "TIE":
      return outcome === "TIE" ? 9.0 : 0;
    case "PLAYER_PAIR":
      return f.playerPair ? 12.0 : 0;
    case "BANKER_PAIR":
      return f.bankerPair ? 12.0 : 0;
    case "ANY_PAIR":
      return f.anyPair ? 6.0 : 0;
    case "PERFECT_PAIR":
      return f.perfectPair ? 26.0 : 0;
    default:
      return 0;
  }
}
...
// ========= 建立「當日(台北)下一局」 =========
async function createNextRoundDaily(
...
export async function GET(req: NextRequest) {
  try {
    const roomCode = String(
      req.nextUrl.searchParams.get("room") || "R60"
    ).toUpperCase();
    ...
    // 3) 計時
    const base = new Date(
      (round as any).startedAt || round.createdAt || new Date()
    );
