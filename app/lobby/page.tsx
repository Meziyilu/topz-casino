...
export default function LobbyPage() {
  ...
  useEffect(() => { refreshMe(); refreshWallet(); }, []);
  ...
  async function doTransfer(action: "deposit" | "withdraw") {
    ...
    } catch (e:any) { setMsg(e.message || "操作失敗"); } finally { setBusy(false); }
  }
    ...
    <main className="min-h-dvh bg-gradient-to-b from-[#0b0f1a] via-[#0a0a0f] to-black text-white">
      <div className="mx-auto max-w-6xl px-4 py-6 flex items-center justify-between">
        <div className="text-xl font-bold tracking-wider">
          <span className="bg-gradient-to-r from-fuchsia-400 to-cyan-400 bg-clip-text text-transparent">TOPZCASINO</span>
          <span className="ml-2 text-white/60 text-sm">大廳</span>
        </div>
        <div className="text-sm text-white/70">
          {me ? (<span>{me.email}{me.isAdmin && <span className="ml-2 rounded bg-amber-400/20 px-2 py-0.5 text-amber-300">Admin</span>}</span>) : (<Link className="underline" href="/login">登入</Link>)}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-6xl px-4 grid lg:grid-cols-3 gap-6">
        ...
        <section className="glass glow-ring rounded-3xl p-6 shadow-2xl">
          ...
          <div className="mt-3 flex gap-3">
            <button disabled={busy} onClick={()=>doTransfer("deposit")}
              className="flex-1 tilt rounded-xl bg-gradient-to-r from-cyan-400 to-emerald-400 py-2 text-black font-semibold hover:opacity-90 active:opacity-80 disabled:opacity-60 transition">
              銀行 → 錢包
            </button>
            <button disabled={busy} onClick={()=>doTransfer("withdraw")}
              className="flex-1 tilt rounded-xl bg-gradient-to-r from-amber-400 to-rose-400 py-2 text-black font-semibold hover:opacity-90 active:opacity-80 disabled:opacity-60 transition">
              錢包 → 銀行
            </button>
          </div>
          ...
          {me?.isAdmin && (
            <div className="mt-6">
              <Link href="/admin" className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-4 py-2 hover:bg-white/10 transition">
                管理面板 →
              </Link>
            </div>
          )}
        ...
        <section className="lg:col-span-2 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            ...
            <Link key={r.code} href={`/casino/${r.code}`} className="sheen tilt glass glow-ring rounded-3xl p-6 shadow-2xl block hover:opacity-95 transition">
              <div className="text-sm text-white/60">房間</div>
              <div className="text-2xl font-bold tracking-wide">{r.name}</div>
              <div className="mt-4 text-white/70 text-sm">點擊進入 →</div>
            </Link>
          ))}
        </section>
      </div>
