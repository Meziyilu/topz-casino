...
const Q = z.object({ room: z.enum(["R30","R60","R90"] as const) });

/** 與 state 同一套 RNG（用 roundId 做種，前後端一致） */
...
function deal(seed: string) {
  ...
  else { if (b2 <= 2) b3 = draw(rand); else if (b2 <= 6 && rand() < 0.5) b3 = draw(rand); }
  ...
  const bankerSix = outcome === "BANKER" && bPts === 6;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (round.phase === "SETTLED") return NextResponse.json({ ok: true, already: true, roundId: round.id });
    ...
    for (const b of bets) {
      if (b.side === "PLAYER" || b.side === "BANKER" || b.side === "TIE") {
        ...
        if (b.side === "TIE" && sim.outcome === "TIE") {
          prize = b.amount * odds.TIE; // 8x（不另外+本金，這個數已是純獎金）
        } else if (b.side === "PLAYER" && sim.outcome === "PLAYER") {
          prize = b.amount * odds.PLAYER;
        } else if (b.side === "BANKER" && sim.outcome === "BANKER") {
          const rate = sim.flags.bankerSix ? 0.5 : odds.BANKER; // 莊 6 點半賠
        ...
        if (prize > 0) {
          // 主注派彩 = 本金 + 獎金 → 贏 100 回 +200
          userPayout[b.userId] = (userPayout[b.userId] ?? 0) + prize + b.amount;
        }
      } else {
        // 側注中獎：賠率×本金（通常不含本金）
        let win = false;
        ...
        if (win) {
          userPayout[b.userId] = (userPayout[b.userId] ?? 0) + Math.floor(b.amount * odds[b.side]);
        }
      }
    }
    ...
    await prisma.$transaction(async (tx) => {
      await tx.round.update({ where: { id: round.id }, data: { phase: "SETTLED", outcome: sim.outcome } });
    ...
    return NextResponse.json({ ok: true, roundId: round.id, outcome: sim.outcome, p: sim.pPts, b: sim.bPts, flags: sim.flags });
  } catch (e) {
